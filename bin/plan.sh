...
for input_mem in `cat ${CONF}/input_mem_${app}`; do
    ...
    for params in `cat ${CONF}/params`; do
        ...
        while [[ ${count} -lt 3 ]]; do
            ...
            rec_count=`ssh ${SLAVE_HOST} cat ${GC_RES_LOG_DIR}/summary_${app}_${input}M_${mem}m_${spark_cores}_${spark_parallelism}_${rdd_compress}_${shuffle_compress}.log | grep ${app} | cut -d -f 7| sed s/[[:space:]]//g`
            echo "${rec_count}"
